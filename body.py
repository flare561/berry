import HTMLParser,re,json,random,requests,datetime,socket,os,sys,oembed,urllib2,urllib,threading,urlparse;
...
if not event.source == self.nickname:
    ...
    if event.command in ['PRIVMSG']:
        ...
        if event.command.lower() in self._prefix('define'):
            event.command = self.config.prefixes[0] + 'wolf'
        ...
        if event.command.lower() in self._prefix('pony'):
            ...
            if datetime.datetime.now(LocalTZ()) < datetime.datetime(year=2013, month=11, day=23, hour=14, minute=00, tzinfo=UTC()):
                ...
                wait = datetime.datetime(year=airdate.year, month=airdate.month, day=airdate.day, hour=14, minute=00, tzinfo=UTC()) - datetime.datetime.now(LocalTZ())
        ...
        if not self.config.raribot:
            ...
            response = ""
            for x in emotes:
                response += 'http://ponymotes.net/view/{} '.format(urllib.quote(x))
            self.send_message(event.respond, response.rstrip())
    
        if event.command.lower() in self._prefix('es') and not self.config.raribot:
            self.send_message(event.respond, 'http://ponymotes.net/view/#{} '.format(urllib.quote(event.params)))
        ...
        if event.command.lower() in self._prefix('qdb'):
            self.send_message(event.respond, "http://adery.net/qdb/?{}".format(event.params))
